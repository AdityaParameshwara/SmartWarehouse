        ...
        function DashboardController($scope, $state, $window, Authentication, $rootScope, $http) {
            ...
            $http.post('/getWarehouseDetails',{"user":Authentication.user.email}).success(function(data){
                ...
                for(var w = 0; w < data.nodes.length; w++){
                    ...
                    for(var n = 0; n < totCnt; n++){
                        ...
                        for(var a = 0; a < data.alerts.length; a++){
                            if(nodeId == data.alerts[a].nodeId && nodeIdParsed.indexOf(data.alerts[a].nodeId) == -1){
                                if(data.alerts[a] =='ThresholdType'){
                                    breachCnt++;
                                }
                                else{
                                    warnCnt++;
                    ...
                    warehouseCount.push({warehouseDetails: details[w], warehouseNodes: data.nodes[w], totalCount: totCnt, breached: breachCnt, warn: warnCnt, safe: (totCnt - (breachCnt+warnCnt))});
                }
            ...
            $scope.warehouseDashboardModal = function (size, selectedWarehouse) {
                document.getElementById('dashboardWarehouseColumn').style.display = 'none';
                document.getElementById('mapColumn').style.display = 'none';
                document.getElementById('warehouseDashboardRow').style.display = 'block';
                $scope.index = selectedWarehouse;
            };
            ...
            $scope.showModal = function(data) {
                ...
                $http.post('/getData', {id: nodeId}).success(function(data){
                    /*nv.addGraph(function () {
                        ...
                        var myData = getChartData();   //You need data...
                        chart.xAxis     //Chart x-axis settings
                            .axisLabel('Timestamp')
                            .tickFormat(function(d){
                                return myData[0].values[d].timestamp;
                            }).rotateLabels(-45);

                        chart.yAxis     //Chart y-axis settings
                            .axisLabel('Moisture Level')
                            .tickFormat(d3.format('.02f'));
                        // document.getElementById("line").innerHTML = '';
                        d3.select('#chartMoisture').append('svg')    //Select the <svg> element you want to render the chart in.
                    ...
                    function getChartData() {
                        ...
                        for(var i = 0; i < data.length; i++){
                            moisture.push({x: i, y:data[i].sensorMoisture});
                        }
                        return [
                            {
                                values: moisture,      //values - represents the array of {x,y} data points
                    ...
                    /*nv.addGraph(function () {
                        ...
                        var myData = getTempData();   //You need data...
                        chart.xAxis     //Chart x-axis settings
                            .axisLabel('Timestamp')
                            .tickFormat(function(d){
                                return myData[0].values[d].timestamp;
                    ...
                    function getTempData() {
                        ...
                        for(var i = 0; i < data.length; i++){
                            chartTemperature.push({x: i, y:data[i].chartTemperature});
                        }
                        return [
                            {
                                values: chartTemperature,      //values - represents the array of {x,y} data points
                    ...
                    /*nv.addGraph(function () {
                        ...
                        var myData = getHumidityData();   //You need data...
                        chart.xAxis     //Chart x-axis settings
                            .axisLabel('Timestamp')
                            .tickFormat(function(d){
                                return myData[0].values[d].timestamp;
                        ...
                        chart.yAxis     //Chart y-axis settings
                            .axisLabel('Humidity Level')
                            .tickFormat(d3.format('.02f'));
                        // document.getElementById("line").innerHTML = '';
                        d3.select('#chartHumidity').append('svg')    //Select the <svg> element you want to render the chart in.
                            .datum(myData)         //Populate the <svg> element with chart data...
                            .call(chart);          //Finally, render the chart!
                    ...
                    function getHumidityData() {
                        var chartHumidity = [];
                        for(var i = 0; i < data.length; i++){
                            chartHumidity.push({x: i, y:data[i].chartHumidity});
                        }
                        return [
                            {
                                values: chartHumidity,      //values - represents the array of {x,y} data points
